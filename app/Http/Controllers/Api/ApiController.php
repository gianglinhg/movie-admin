...
class ApiController extends Controller
{
    ...
    public function get_movies_of_type($type, Request $request)
    {
        $select = ['id', 'name', 'origin_name', 'slug', 'thumb_url', 'poster_url', 'publish_year', 'episode_time'];
        $page = (int)($request->get('page', 1));
        $total = (int)($request->get('total', 100));

        if ($type === 'hoat-hinh') {
            $categoryId = Category::where('slug', $type)->value('id');
            $movies = Movie::select($select)
            ...
            $movieType = $movieTypeMap[$type] ?? '';
            $query = Movie::select($select)
                ->when($movieType, fn($query) => $query->where('type', $movieType))
                ->latest()
                ->skip(($page - 1) * $total)
                ->take($total)
                ->get();
